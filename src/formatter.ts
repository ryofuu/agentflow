...
export function printJobStart(jobId: string, displayName?: string): void {
  const label = displayName && displayName !== jobId ? `${jobId} (${displayName})` : jobId;
...
export function printStateChange(jobId: string, from: string, to: string, duration?: number): void {
  const durationStr = duration !== undefined ? ` (${duration.toFixed(1)}s)` : "";
  console.log(
    `${DIM}[state] ${jobId}: ${from} → ${to}${durationStr}${RESET}`,
  );
}

export function printError(jobId: string, stepName: string, exitCode: number): void {
  console.log(
...
export function printParallelHeader(jobIds: string[]): void {
  console.log(
    `${BOLD}── ${jobIds.join(", ")} ──── (parallel)${RESET}`,
  );
}
